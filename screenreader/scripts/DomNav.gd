	...
	for c in list:
		if c is Array:
			...
			if get_node_type(NODE_TYPE.INTERACT_NODE):
				...
				if !end_node_branches.has(list):
					end_node_branches.append(list)
